...
def calculate_all_features(page):
    ...
    if page.win_start >= page.win_end:
        ...
        return {}, {}       # change from None to {} to keep instances for default type where all
                            # pages are needed for a complete report
    elif page.win_start < page.time_start:
        print('Window start time cannot be earlier than page start time!')
        return None, None
    elif page.win_end > page.time_end:
        print('Window end time cannot be later than page end time!')
        return None, None
...
def calculate_single_eye(page, eye):
    ...
    if series is not None and not series.dropna().empty:
        ibi_baseline = series.median()
    else:
        ibi_baseline = np.nan  # Or some fallback value like 0
    # truncate dataframes in the page object for calcuation
    dfSamples, dfFix, dfSacc, dfBlink = truncate_df_by_time(page.dfSamples, page.dfFix, page.dfSacc, page.dfBlink, page.win_start, page.win_end)
    ...
    res['ibi'] = dfSamples['interblink_interval'].mean()
    res['ibi_baseline'] = ibi_baseline
    ...
    # get raw pupil size at the moment of the center of the window
    if np.isnan(page.win_start) or np.isnan(page.win_end):
        raw_pupil = np.nan
    else:
        time = int((page.win_start*1000 + page.win_end*1000)/2)
        row_ = pupil[dfSamples['tSample']==time]
        raw_pupil = row_.values[0] if not row_.empty else np.nan
    ...
    # save to results dict
    res['pupil_baseline'] = pupil_baseline
    res['pupil'] = raw_pupil
    res['pupil_mean'] = pupil_mean
    res['pupil_slope'] = slope
    # normalize pupil features
    if pd.notna(pupil_baseline) and pupil_baseline != 0:
        res['page_norm_pupil'] = raw_pupil / pupil_baseline if pd.notna(raw_pupil) else np.nan
        res['page_norm_pupil_mean'] = pupil_mean / pupil_baseline if pd.notna(pupil_mean) else np.nan
    else:
        res['page_norm_pupil'] = np.nan
        res['page_norm_pupil_mean'] = np.nan
...
def calculate_word_fix_corr(dfFix):
    ...
    # only the first fixation
    dfFix = dfFix.sort_values(by=['fixed_word_index', 'tStart'])
    # Drop duplicates — keeping the first fixation on each word
    dfFix = dfFix.drop_duplicates(subset='fixed_word_index', keep='first')
