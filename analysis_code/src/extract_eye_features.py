...
from .utils import preprocess_pupil, calc_interblink_interval
...
def extract_subject_features(sub_folder, win_type, task_type, alpha, is_plot=False):
    ...
    task_type: str
        Type of simluated task paradigm to use (sr for self-report and tp for tought-probe)
    ...
    if 'slide' in win_type:
        ...
        wlen = float(win_type.replace('slide', ''))
        # take care of naming conventions (1.0 -> 10)
        if wlen >= 10:
            wlen = wlen / 10
        time_offset = 5
        # extend window for self-report event
        time_offset_sr = 5 + alpha + wlen/2
        page_time_offset = time_offset + wlen / 2
        step = 0.25
        ...
        for page in tqdm(all_pages, desc="Calculating features on each page"):
            if page.mw_reported:
                ...
                process_sliding_window(page, mid_point, 'MW_onset', wlen, time_offset, step, all_res_left, all_res_right)

                # Process MW offset
                # mid_point = page.mw_offset
                # process_sliding_window(page, mid_point, 'MW_offset', wlen, time_offset, step, all_res_left, all_res_right)

                # Process self-report event (offset)
                mid_point = page.time_end
                process_sliding_window(page, mid_point, 'self_report', wlen, time_offset_sr, step, all_res_left, all_res_right)

            else:
                # Process control for MW onset
                ...
                process_sliding_window(page, mid_point, 'control_onset', wlen, time_offset, step, all_res_left, all_res_right)

                # Process control for self-report
                mid_point = page.time_end
                process_sliding_window(page, mid_point, 'control_sr', wlen, time_offset_sr, step, all_res_left, all_res_right)

         # Convert results to DataFrames
        df_L, df_R = pd.DataFrame(all_res_left), pd.DataFrame(all_res_right)

        # Sanitize win_type to remove dot from float
        # if '.' in str(win_type):
        #     win_str = str(win_type).replace('.', '')

        file_name_L = f'{sub_folder}/s{sub_id}_L_features_{win_type}.csv'
        file_name_R = f'{sub_folder}/s{sub_id}_R_features_{win_type}.csv'
    
    else:
        print('Computing window start and end times for each page...')
        # simulate thought probes
        if task_type == 'tp':
            # call function with input alpha to get simulated thought probes before the response time 
            all_pages = simulate_thought_probes(all_pages,alpha)
            # change alpha to zero since there's no response artifact in simulated probes
            alpha = 0

        # call function to compute the window time for feature extraction
        all_pages = compute_window_time(all_pages, win_type, alpha)
        # feature extraction
        print('Extracting and calculating eye features...')
        df_L, df_R = compute_features(all_pages)
        file_name_L = f'{sub_folder}/s{sub_id}_L_features_{win_type}_{task_type}.csv'
        file_name_R = f'{sub_folder}/s{sub_id}_R_features_{win_type}_{task_type}.csv'
    ...
    if not df_L.empty:
        df_L = df_L.sort_values(by=['run', 'page'], ascending=[True, True]).reset_index(drop=True)
        df_L.to_csv(file_name_L, index=False)
        print('Saved Data for L Eye.')

    if not df_R.empty:
        df_R = df_R.sort_values(by=['run', 'page'], ascending=[True, True]).reset_index(drop=True)
        df_R.to_csv(file_name_R, index=False)
...
def simulate_thought_probes(all_pages, alpha):
    """
    Simulates thought probe timing for a list of pages.

    Adjusts the `time_end` and `page_dur` of each page by subtracting a constant alpha.
    If a mind-wandering (MW) report is present, the thought probe is randomly placed 
    between the MW onset and the adjusted page end. Otherwise, it is placed between 
    the page start and the adjusted page end.

    Parameters:
        all_pages (list): List of page objects
    Returns:
        list: The updated list of page objects with modified `time_end` values.
    """
    random.seed(42)  # Set seed for reproducibility

    for page in all_pages:
        # Adjust the page duration and end time
        page.time_end -= alpha

        # Determine thought probe timing
        if page.mw_reported:
            thought_probe = random.uniform(page.mw_onset, page.time_end)
        else:
            thought_probe = random.uniform(page.time_start+5, page.time_end)

        # Update the page end time to the thought probe time
        page.time_end = thought_probe
        page.page_dur = page.time_end - page.time_start
...
def compute_window_time(pages, win_type, alpha):
    '''
    ...
    # mindless reading sample: mw onset -> mw offset
    if win_type == 'default':
        for page in pages:
            # MW sample
            if page.mw_reported:
    ...
    # mw_full type
    # normal reading sample: page start -> page end - alpha
    # mindless reading (MW) sample: mw onset -> page end - alpha
    elif win_type == 'mw_full':
        # loop thru all pages and update win start/end info
        # use self-report - alpha as mw offset instead of reported offset
        page.time_end -= alpha
        page.page_dur -= alpha
        ...
        for page in pages:
            # MW sample
            if page.mw_reported:
                page.win_start, page.win_end = page.mw_onset, page.time_end
            # normal sample
            else:
                page.win_start, page.win_end = page.time_start, page.time_end
        return pages
    ...
    # NR sample: random start -> random end on non reported page 
    # MR sample: mw onset -> page end - alpha
    # winlen(NR) == winlen(MR)
    elif win_type == 'mw_fixed':
        ...
        for page in pages:
            ...
            if page.mw_reported:
                page.mw_offset = page.time_end
                page.mw_dur = page.mw_offset - page.mw_onset
                if page.mw_dur >= 2:
                    page.win_start = page.mw_onset
                    page.win_end = page.mw_offset
                    pages_mw.append(page)
        ...
        for index, page_nr in enumerate(pages_nr):
            ...
            # use the time between page start and mw start for selecting control start if the page is long enough. otherwise, randomly select one time point from the reading page
            ...
            # find the fixation that is closest less or equal to win_start
            tStart = np.array(page_nr.dfFix['tStart']) / 1000
            # Filter values that are less than or equal to win_start
            valid_values = tStart[tStart <= win_start]
            if len(valid_values) > 0:
                # Find the one closest to win_start
    ...
    # page_fixed type: Matches MR and NR page length
    # NR sample: page start -> random end on non-reported page
    # MR sample: page start -> page end -alpha on reported page
    # winlen(NR) == winlen(MR)
    elif win_type == 'page_fixed':
        # Initialize empty lists to store page objects
        pages_nr = []   # List of normal reading pages
        pages_mw = []   # List of mind-wandering pages

        # Categorize pages based on whether mind-wandering is reported
        for page in pages:
            page.time_end -= alpha
            page.page_dur -= alpha

            if page.mw_reported:
                pages_mw.append(page)  # Add to mind-wandering pages
            else:
                pages_nr.append(page)  # Add to normal reading pages

        # Check if there are no mind-wandering pages
        if len(pages_mw) == 0:
            print('No mind-wandering instances are found! Throw out this subject from analysis.')
            return []  # Return an empty list if no mind-wandering pages are found

        # Loop through each normal reading page and define the time window based on mind-wandering
        # samples from all 5 runs. Adjust window start and end accordingly.
        for index, page_nr in enumerate(pages_nr):
            # Assign a corresponding mind-wandering page (cyclically if there are fewer MW pages than NR pages)
            page_mw = pages_mw[index % len(pages_mw)]
            
            # Set the window start and end for normal reading pages
            page_nr.win_start = page_nr.time_start
            page_nr.win_end = min(page_nr.time_end, page_nr.win_start + page_mw.page_dur)

        # Set the window start and end for all mind-wandering pages
        for page_mw in pages_mw:
            page_mw.win_start = page_mw.time_start
            page_mw.win_end = page_mw.time_end

        # Return the list of normal reading pages followed by the mind-wandering pages
        return pages_nr + pages_mw
    

    # 2/5 seconds from the end of each page
    # NR sample: -2/5s-> page end - alpha on non-reported page
    # MR sample: -2/5s-> page end - alpha on reported page
    # winlen(NR) == winlen(MR) == 2/5
    elif 'end' in win_type:
        wlen = float(win_type.replace('end', ''))
        # loop thru all pages and update win start/end info
        for page in pages:
            page.time_end -= alpha
            page.page_dur -= alpha
            page.win_end = page.time_end
            page.win_start = page.time_end - wlen
        return pages
...
def fill_dict(res, page, verbose=True):
    ...
    if verbose:
        ...
        res['mw_onset'] = page.mw_onset
        res['mw_offset'] = page.mw_offset
...
def process_sliding_window(page, mid_point, label, wlen, time_offset, step, all_res_left, all_res_right):
    """
    ...
    b_time = (mid_point-wlen/2) - page.time_start
    backward_steps = min(total_steps, int(b_time / step))

    f_time = page.time_end - (mid_point+wlen/2)
    forward_steps = min(total_steps, int(f_time / step))

    left = mid_point - backward_steps * step - wlen / 2
    right = mid_point + forward_steps * step - wlen / 2

    # Iterate over sliding window steps
    for win_start in np.arange(left, right+step, step):
        # handle precision issues
        win_start = win_start.round(6)
        if win_start < page_copy.time_start:
            continue
        elif win_start > right or win_start+wlen > page_copy.time_end:
            break

        # assign window start and end time
        page_copy.win_start = win_start
        page_copy.win_end = win_start + wlen
