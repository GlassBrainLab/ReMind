...
#                                 FUNCTIONS
# ==============================================================================
def get_line(mouse_y, page_bottom, page_height, line_count=16):
    # This function returns the number of line that's the closest to the click
    #
    # INPUTS:
    #   mouse_y: y position of the mouse click
    #   page_bottom: bottom position of displayed page
    #   page_height: height of displayed page
    #   line_count: number of lines per page
    #
    # OUTPUTS:
    #   line: an int to indicate the line number
    line = round((1 - (mouse_y-page_bottom) / page_height) * line_count) + 1
    return line
# ==============================================================================
#                               END FUNCTIONS
# ==============================================================================
...
expInfo = {'participant': '0', 'session': '1','type': ['self-report'],'is_tracker_connected': False, 'is_eeg_collected': False}
...
if expInfo['type'] == 'eye tracking':
    ...
    instructionsFile = 'instructions_self_report.xlsx'
    advanceKeys = ['space']
    errorKey = 'f'
    triggerKey = 't'
    wait_message = f'That’s the end of calibration.\n\nPlease place your left index finger on F and left thumb on the Spacebar.\n\nExperimenter, record EEG data and press {triggerKey} to begin the experiment.'
    resetKeys = ['backspace']
    enable_probes = False
...
# send events to biosemi if eeg is collected
send_eeg_event = expInfo['is_eeg_collected']
...
filename = _thisDir + os.sep + u'data/%s_R%s_%s_%s' % (expInfo['participant'], expInfo['session'], expName, expInfo['date'])
...
fade_dur = 5
error_count = 3
task_start_time = 5         # task start time in seconds
# define line count to compute the vetrices of each line
line_count = 16 # line per page
...
instructions_image.borderColor = [0,0,0] # remove distracting border line
...
reading_image_error = visual.ImageStim(
    ...
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-1.0, units='height')
text_error_bottom = visual.TextStim(win=win, name='text_error_bottom',
    text='',
    font='Open Sans',
    pos=(-.75, .1), height=instructions_height/2, wrapWidth=.1, ori=0.0,
    color='white', colorSpace='rgb', opacity=None,
    languageStyle='LTR',
    depth=-2.0);
...
# compute the position of displayed page
page_width,page_height = reading_image_error.size
page_bottom = reading_image_error.pos[1]-page_height/2
page_left = reading_image_error.pos[0]-page_width/2
page_right = reading_image_error.pos[0]+page_width/2
line_height = page_height/line_count
# declare the offset for highlighted line area
# it shifts line area up to the middle of each line
line_height_offset = line_height/4
line_y = (np.arange(line_count)+0.5)*line_height + page_bottom + line_height_offset # vertical position of each line

# declare three rectangles
top_rect = visual.Rect(win=win, units='height', colorSpace='rgb',
                       lineColor=None, fillColor=(1, 1, 0.0000),
                       height=line_height, opacity=0.3, depth=-4)
mid_rect = visual.Rect(win=win, units='height', colorSpace='rgb',
                       lineColor=None, fillColor=(1, 1, 0.0000),
                       width=abs(page_right-page_left), opacity=0.3, depth=-4)
bot_rect = visual.Rect(win=win, units='height', colorSpace='rgb',
                       lineColor=None, fillColor=(1, 1, 0.0000),
                       height=line_height, opacity=0.3, depth=-4)
rects = [top_rect, mid_rect, bot_rect]
...
for thisInstructionsLoop in instructionsLoop:
    ...
    while continueRoutine and enable_instructions:
        # get current time
...
if instrReps:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    # get the page number of practice reading
    page_count = 3
    ...
    trials = data.TrialHandler(nReps=1.0, method='sequential',
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('practice/halteres.xlsx'),
    ...
    for thisTrial in trials:
        ...
        elif (page_index == page_count) and (not is_MW_reported):
            do_probes = 1
            detect_error = True
        else:
            detect_error = False
        ...
        for thisProbesLoop in probesLoop:
            ...
            if expInfo['type'] == 'self-report':
                text_error_bottom.setText('Click to highlight the text where your mind wandered (if any), then press SPACE.')
            else:
            ...
            # declare click circles
            mouse_click_circles = [visual.Circle(win=win, lineColor='black', fillColor='green', size=0.01, depth=-3, units='height'),
                                   visual.Circle(win=win, lineColor='black', fillColor='blue', size=0.01, depth=-3, units='height')]
            mouse_click_index = 0
            mouse_click_started_time = []
            ...
            while (continueRoutine and detect_error):
                ...
                if reading_image_error.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    ...
                    if buttons != prevButtonState:  # button state changed?
                        ...
                        if sum(buttons) > 0:  # state changed to a new click
                            ...
                            if gotValidClick:  # abort routine on response
                                ...
                                # log every click started time
                                mouse_click_started_time.append(t)

                                # reset the click index so that only 2 circles can be drawn
                                if mouse_click_index >= len(mouse_click_circles):
                                    mouse_click_index -= len(mouse_click_circles)
                                # draw a circle at valid mouse click
                                win.units = 'height'
                                x, y = mouse_error.getPos()
                                mouse_click_circle = mouse_click_circles[mouse_click_index]
                                mouse_click_circle.setPos((x,y))
                                mouse_click_circle.setAutoDraw(True)
                                gotValidClick = False
                                x_pos[mouse_click_index] = x #/win.size[1]
                                y_pos[mouse_click_index] = y #/win.size[1]
                                # update mouse click index
                                mouse_click_index += 1

                                if mouse_click_index == 1:
                                    for rect in rects:
                                        rect.setAutoDraw(False)
                                # highlight the area between two circles by drawing a line
                                elif mouse_click_index == len(mouse_click_circles):
                                    # get the line number of each click
                                    line1 = get_line(y_pos[0], page_bottom, page_height)
                                    line2 = get_line(y_pos[1], page_bottom, page_height)
                                    lines = [line1, line2]
                                    # two clicks are at the same link_event_flags
                                    if line1 == line2:
                                        rect_x = (x_pos[0] + x_pos[1]) / 2
                                        rect_y = line_y[16-line1]

                                        top_rect.pos = (rect_x, rect_y)
                                        top_rect.width = abs(x_pos[1] - x_pos[0])
                                        top_rect.setAutoDraw(True)

                                    # otherwise get the position of top and bottom click first
                                    else:
                                        top_index = np.argmax(y_pos)
                                        bot_index = np.argmin(y_pos)

                                        # two clicks are off by 1 line
                                        # top click
                                        top_x = x_pos[top_index]
                                        top_y = y_pos[top_index]
                                        top_line = lines[top_index]
                                        # set up the top rectangle
                                        # width: click x -> page right
                                        # height: one line height
                                        top_rect.pos = ((top_x+page_right)/2, line_y[16-top_line])
                                        top_rect.width = abs(page_right - top_x)
                                        top_rect.setAutoDraw(True)

                                        # bottom click
                                        bot_x = x_pos[bot_index]
                                        bot_y = y_pos[bot_index]
                                        bot_line = lines[bot_index]
                                        # set up the bottom rectangle
                                        # width: page left -> click x
                                        # height: one line height
                                        bot_rect.pos = ((bot_x+page_left)/2, line_y[16-bot_line])
                                        bot_rect.width = abs(bot_x - page_left)
                                        bot_rect.setAutoDraw(True)

                                        # set up the middle rectangle if two clicks are off more than 1 line
                                        if abs(line1-line2) > 1:
                                            mid_rect.pos = (0, (line_y[15-top_line]+line_y[17-bot_line])/2)
                                            mid_rect.height = line_height * (abs(line1-line2)-1)
                                            mid_rect.setAutoDraw(True)
                ...
                if key_resp_error.status == STARTED and not waitOnFlip:
                    # check whether BACKSPACE is pressed
                    resetPressed = key_resp_error.getKeys(keyList=resetKeys, waitRelease=False)
                    if 'backspace' in resetPressed:
                        # remove circles
                        for circle in mouse_click_circles:
                        ...
                        # clean pressed keys
                        key_resp_error.clearEvents()
                        # reset mouse click index
                        mouse_click_index = 0
        ...
        for thisRepeatLoop in repeatLoop:
            ...
            key_resp.rt = []
            _key_resp_allKeys = []
            text_page_bottom.setText('Press SPACE to continue or F to report an error.\n')
            repeat_page = 0
            # keep track of which components have finished
            trialComponents = [reading_image, cross, key_resp, text_page_bottom]
            for thisComponent in trialComponents:
    ...
    if (expInfo['type']=='eye tracking' or 'self-report') or expInfo['type']=='eeg':
        ...
        questionsLoop = data.TrialHandler(nReps=1.0, method='sequential',
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('practice/halteres_questions.xlsx'),
...
for thisReadingLoop in readingLoop:
    ...
    # the eyelink calibration routine was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    routineTimer.add(task_start_time)
    # update component parameters for each repeat
    # keep track of which components have finished
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if beginning_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > beginning_text.tStartRefresh + task_start_time-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisTrial in trials:
        ...
        if (key_resp.keys == errorKey) and (expInfo['type'] == 'eye tracking' or 'self-report'):
            ...
            # === EYELINK SEND MESSAGE ON SCREEN CHANGE === #
            el_tracker.sendMessage('MW Reported')
            # === EEG MESSAGE ===
            if send_eeg_event:
                ser.write(bytearray([40]))

        else:
            # uncomment to enable report after every page
            # if expInfo['type'] == 'self-report':
            #     detect_error = True
            # else:
            #     detect_error = False
        ...
        for thisProbesLoop in probesLoop:
            ...
            if continueRoutine and detect_error:
                ...
                if sum(buttons):
                    ...
                    for obj in [reading_image_error,]:
                        if obj.contains(mouse_error):
                            gotValidClick = True
                            mouse_error.clicked_name.append(obj.name)
                probesLoop.addData('first_click.x', x_pos[0])
                probesLoop.addData('first_click.y', y_pos[0])
                probesLoop.addData('second_click.x', x_pos[1])
                probesLoop.addData('second_click.y', y_pos[1])
                # log the click started time in the csv file
                all_click_time = '/'.join(np.array(mouse_click_started_time, dtype='str'))
                probesLoop.addData('mouse_click_time', all_click_time)
            ...
            if continueRoutine and enable_probes:
                # === EYELINK SEND MESSAGE ON SCREEN CHANGE === #
            ...
            # ==================================================================
            # -------Run Routine "probes"-------
            while (continueRoutine and enable_probes):
                # get current time
    ...
    for thisNewReadingLoop in newReadingLoop:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if next_reading_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > next_reading_text.tStartRefresh + 1-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_resp_next_reading.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_resp_next_reading.tStartRefresh + 1-frameTolerance:
                    # keep track of stop time/frame for later
