...
expName = 'ReadingOnline'  # from the Builder filename that created this script
...
pages = [32, 29, 33]
# error types
page_type = ['lexical', 'syntactic', 'semantic', 'discourse', 'gibberish', 'control', 'default']
# reading task - TODO: WHAT DOES THIS DO???
reading_task = 3
...
beginning_text = visual.TextStim(win=win, name='beginning_text',
    text='That’s the end of calibration.\n\nYour reading task will start in seconds. Good luck!',
    font='Open Sans',
...
text_page_bottom = visual.TextStim(win=win, name='text_page_bottom',
    text='',
    font='Open Sans',
    pos=(0, -.475), height=instructions_height/2, wrapWidth=1.3, ori=0.0,
    color='white', colorSpace='rgb', opacity=None,
    languageStyle='LTR',
    depth=-4.0);
...
next_reading_text = visual.TextStim(win=win, name='next_reading_text',
    text='That’s the end of the comprehensive questions.\n\nYou will be reading another passage. In 15 seconds, reading will start, or press SPACE to continue.',
    font='Open Sans',
    pos=(0, 0), height=instructions_height, wrapWidth=None, ori=0.0,
    color='white', colorSpace='rgb', opacity=None,
    languageStyle='LTR',
...
goodbye_text_top = visual.TextStim(win=win, name='goodbye_text_top',
    text='Thank you for participating!\n\nPlease enter the following  code ino your MTurk HIT:',
    font='Open Sans',
    pos=(0, 0.35), height=instructions_height, wrapWidth=1.3, ori=0.0,
    color='white', colorSpace='rgb', opacity=None,
    languageStyle='LTR',
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if beginning_text.status == STARTED:
        ...
        if tThisFlipGlobal > beginning_text.tStartRefresh + 10-frameTolerance:
...
readingLoop = data.TrialHandler(nReps=1.0, method='sequential',
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('reading_condition_file.xlsx'),
...
for thisReadingLoop in readingLoop:
    ...
    p2 = p1 + error_fs
    while p2 < page_count:
        error_index = r.randint(p1, p2)
        p1 = error_index + 1
        p2 = p1 + error_fs
        error_page.append(error_index)
    # generate the error type for each error page
    error_type = [i % error_count for i in range(len(error_page))]
    r.shuffle(error_type)
    ...
    if readingLoop.nRemaining != 0:
        readingnReps = 1
        if reading_task == 2 and readingLoop.nRemaining == 1:
            isNextReading = 0
        else:
            isNextReading = 1
    else:
    ...
    for thisTrialLoop in trialLoop:
        ...
        for thisTrial in trials:
            ...
            error_type_int = 6
            # error page
            if trials.thisTrialN == error_page[error_page_index]:
                do_probes = 1
                error_type_int = error_type[error_page_index]
                if error_type_int == 0:
                    image_displayed = image_lexical
                elif error_type_int == 1:
                    image_displayed = image_syntactic
                elif error_type_int == 2:
                    image_displayed = image_semantic
                elif error_type_int == 3:
                    image_displayed = image_discourse
                elif error_type_int == 4:
                ...
                if error_page_index < (len(error_page)-1):
                    error_page_index += 1
            # update log
            thisExp.addData('error_type', page_type[error_type_int]);
            # keep track of which components have finished
            ...
            while continueRoutine:
                ...
                if key_resp.status == STARTED and not waitOnFlip:
                    theseKeys = key_resp.getKeys(keyList=['space', 'x'], waitRelease=False)
                ...
                # send *cross* updates to eye tracker
                if cross.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance: # when it's about to be displayed
                    win.callOnFlip(el_tracker.sendMessage, 'displayed cross')  # send msg at next scr refresh

                # send *reading_image* updates to eye tracker
                if reading_image.status == NOT_STARTED and tThisFlip >= fix_dur-frameTolerance: # when it's about to be displayed
                    win.callOnFlip(el_tracker.sendMessage, f'displayed {image_displayed}')  # send msg at next scr refresh with image filename
            ...
            for thisProbesLoop in probesLoop:
                ...
                while continueRoutine:
                    ...
                    if key_resp_error.status == STARTED and not waitOnFlip:
                        theseKeys = key_resp_error.getKeys(keyList=['space'], waitRelease=False)
                    ...
                    # This code sends eyelink messages when the screen is about to change. The if statements are borrowed from the
                    # psychopy auto-generated python code for the times when each stimulus is supposed to start.
                    # Be sure to edit the start times (0.0, cross_dur below) to match those of the stimuli in your routine.

                    # send *reading_image_error* updates to eye tracker
                    if reading_image_error.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance: # when it's about to be displayed
                        win.callOnFlip(el_tracker.sendMessage, 'displayed errorimage {image_displayed}')  # send msg at next scr refresh
                ...
                while continueRoutine:
                    ...
                    if text_probe_instructions.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance: # when it's about to be displayed
                        win.callOnFlip(el_tracker.sendMessage, 'displayed probes')  # send msg at next scr refresh
            ...
            # set up handler to look after randomisation of conditions etc
            repeatLoop = data.TrialHandler(nReps=do_probes, method='sequential',
                extraInfo=expInfo, originPath=-1,
                trialList=[None],
        ...
        while continueRoutine:
            ...
            if key_resp_instructions_2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_instructions_2.getKeys(keyList=['space'], waitRelease=False)
        ...
        for thisComponent in comp_instructionsComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        trialLoop.addData('text_instructions_top_2.started', text_instructions_top_2.tStartRefresh)
        trialLoop.addData('text_instructions_top_2.stopped', text_instructions_top_2.tStopRefresh)
        trialLoop.addData('instructions_image_2.started', instructions_image_2.tStartRefresh)
        trialLoop.addData('instructions_image_2.stopped', instructions_image_2.tStopRefresh)
        trialLoop.addData('text_instructions_bottom_2.started', text_instructions_bottom_2.tStartRefresh)
        trialLoop.addData('text_instructions_bottom_2.stopped', text_instructions_bottom_2.tStopRefresh)
        # check responses
        if key_resp_instructions_2.keys in ['', [], None]:  # No response was made
            key_resp_instructions_2.keys = None
        trialLoop.addData('key_resp_instructions_2.keys',key_resp_instructions_2.keys)
        if key_resp_instructions_2.keys != None:  # we had a response
            trialLoop.addData('key_resp_instructions_2.rt', key_resp_instructions_2.rt)
        trialLoop.addData('key_resp_instructions_2.started', key_resp_instructions_2.tStartRefresh)
        trialLoop.addData('key_resp_instructions_2.stopped', key_resp_instructions_2.tStopRefresh)
        ...
        for thisQuestionsLoop in questionsLoop:
            ...
            while continueRoutine:
                ...
                if key_resp_question.status == STARTED and not waitOnFlip:
                    theseKeys = key_resp_question.getKeys(keyList=['1', '2', '3', '4'], waitRelease=False)
                ...
                if text_question.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance: # when it's about to be displayed
                    win.callOnFlip(el_tracker.sendMessage, 'displayed comprehensive questions')  # send msg at next scr refresh
        ...
        for thisNewReadingLoop in newReadingLoop:
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                if key_resp_next_reading.status == STARTED and not waitOnFlip:
                    theseKeys = key_resp_next_reading.getKeys(keyList=['space'], waitRelease=False)
    ...
    # completed readingnReps repeats of 'trialLoop'
